...
void processData(float* input, float*past_output, float*new_output, int &count, CArray w_data, double &db)
{
    //if(count == 0)
    //{
   ...
   // }
    ...
    for (int i =0; i < BUFFER_SIZE; i++)
    {
        m_data[i] = (pow(10, log10(Emag[i]) + db/20)) * w_data[i];
    }
    ifft(m_data);
 ...
 */
void dsp(char* pcm_ds_input_buffer, mutex &readMutex, char* pcm_ds_output_buffer, mutex &writeMutex, bool &input1_ready, bool &output1_ready, bool &output2_ready, bool &output1_change, char* temp_output_buffer, float* whitenoise, double &db)
{
    ...
    while (1)
    {
        ...
        if (input1_ready)
        {
            ...
            processData(float_input,float_past_output,float_new_output,count,w_data,db);
