...
int main(int argc, char *argv[])
{
    ...
    bool power = false;
    double db = +3.0;
    bool input1_capture = false;
    bool input1_ready = false;
    bool output1_change = false;
    bool output1_ready = true;
    ...
    threads.push_back(thread(readMicrophone, pcm_ds_input_buffer, ref(readMutex), ref(input1_ready), ref(output1_ready), ref(input1_capture), ref(power)));

    // Thread for Digital Signal Processing
    threads.push_back(thread(dsp, pcm_ds_input_buffer, ref(readMutex), pcm_ds_output_buffer, ref(writeMutex), ref(input1_ready), ref(output1_ready), ref(output2_ready), ref(output1_change), temp_output_buffer, whiteNoise, ref(db)));
    ...
    threads.push_back(thread(writeSpeaker, pcm_ds_output_buffer, ref(writeMutex), ref(input1_ready), ref(output1_ready), ref(input1_capture), ref(output2_ready), ref(readMutex), temp_output_buffer, ref(power)));

    threads.push_back(thread(parseMQTT, ref(db), ref(dbMutex), ref(power), ref(powerMutex)));
